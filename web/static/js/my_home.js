        ...
        success:function (result) {
            ...
            for(var i = 0; i < objarr.length; i++){
                ...
                var li = $("<li><div class='article'><p class='article-title'>" +
                    ...
                    " <span class='publish-status'>"+status+"</span> <a href='"+editPath+"'>编辑</a></div></div></li>");
                $("#articles").append(li);
            }
        },
        error:function (msg) {
    ...
    $.ajax({
        ...
        success:function (result) {
            ...
            if(listRead.length === 0){
                var h5 = $("<h5>你还没有发表过文章哦</h5>")
                $("#read-top-ul").append(h5);
            }
            if(labels.length === 0){
                var h5 = $("<h5>你还没有添加过个性标签</h5>")
                $("#tag-ul").append(h5);
            }
